...
2013: Also, use statsmodels' pydta package? (No; it was abandoned)
...
Text files are saved using UTF8.
"""
from surveypandas import defaults # The idea is that surveypandas should only call surveypandas_config once. Other modules (e.g. RDC, Gallup code) may have overwritten surveypandas's defaults.
...
'labellist',   : raw text from a Stata label list command [not yet]
...
##             self.update(dict(args[0]))
    ...
    ################################################################
        ...
        tsvs=tsvToDict(datafilepath+'.tsv', utf8=True)
        cbook=dict([[avd['varname'],avd]  for  avd in tsvs])

        self.update(cbook)
        if 0:
            ffffffffffffffffxxx,"make sure this is in utf-8?"
        self.__orderedVarNames=[LL[1] for LL in open(datafilepath+'.tsv','rt', encoding='utf8').readlines()[1:]]
        return
