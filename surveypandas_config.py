...
def main():
    ...
    if 'bin/surveypandas' not in os.getcwd():
        # 201709: No complaints for a year; so this caution was suppressed
        #print(' Caution: new code October 2016: chdir in surveypandas_config. Not sure why this was not done before. Complaints to CPBL. chdir(%s)'%dir_path)
        os.chdir(dir_path)

    repoFile=(repoPath if repoPath else '.')+'/config.cfg'
    repoTemplateFile=(repoPath if repoPath else '.')+'/config-template.cfg'

    print('surveypandas setting defaults:')
